...
/*
import com.deadlockarena.constant.AppPrincipalFrame;
...
public class AnimationAndSound {
	...
	public void playSound(String soundType) {
		if (AppPrincipalFrame.soundButton.isOn()) {
					...
					for (String fileString : soundtracks) {
						try {
							...
							while (inputStreamMusic.available() > 0) {
								if (!AppPrincipalFrame.loopButton.isOn() || !AppPrincipalFrame.musicButton.isOn() || changedMusic)
									return;
	...
	private void initMusic() {
		if (AppPrincipalFrame.soundtrack == null)
			return;
		try {
			inputStreamMusic = AudioSystem.getAudioInputStream(new File("music/" + AppPrincipalFrame.soundtrack + ".wav"));
		} catch (Exception exc) {
			exc.printStackTrace();
		}
	}
	...
	private void startMusic() {
		if (AppPrincipalFrame.musicButton.isOn()) {
			try {
	...
	private void controlVolume(Clip c, float f) { FloatControl gainControl = (FloatControl) c.getControl(FloatControl.Type.MASTER_GAIN); gainControl.setValue(f); 
	// Reduce volume by f decibels. 
	 ...
	 private void initSoundTracks() {
		...
		AppPrincipalFrame.soundtrack = soundtracks[AppPrincipalFrame.random.nextInt(soundtracks.length)];
	}
	 ...
	 private String selectSount(String soundType) {
		...
		switch (soundType) {
		case "melee":
			switch (AppPrincipalFrame.random.nextInt(2)) {
			case 0:
				sound = "melee1";
				break;
			case 1:
